...
const useLogsModel = () => {
  ...
  const extractThread = (line) => {
    // Look for the pattern: [module:line] [threadId] [processId]
    // Example: [PacketTunnelProvider:2436] [30472355] [51891]
    const multipleBoacketsMatch = line.match(/\[[^\]]+:\d+\]\s*\[(\d+)\]\s*\[(\d+)\]/);
    if (multipleBoacketsMatch) {
      return multipleBoacketsMatch[1]; // Return thread ID (first number after module)
    }

    // Look for combined pattern [thread:process] 
    const threadProcessMatch = line.match(/\[(\d+):(\d+)\]/);
    if (threadProcessMatch) {
      return threadProcessMatch[1]; // Return thread ID
    }

    // Look for thread-specific patterns
    const threadPatterns = [
      /thread[:\s]+(\d+)/i,
      /tid[:\s]+(\d+)/i,
      /\[(\d+)\]/,  // Generic bracket pattern (fallback)
    ];
  ...
  const extractProcess = (line) => {
    ...
    const multipleBoacketsMatch = line.match(/\[[^\]]+:\d+\]\s*\[(\d+)\]\s*\[(\d+)\]/);
    if (multipleBoacketsMatch) {
      return multipleBoacketsMatch[2]; // Return process ID (second number after module)
    }

    // Look for combined pattern [thread:process]
    const threadProcessMatch = line.match(/\[(\d+):(\d+)\]/);
    if (threadProcessMatch) {
      return threadProcessMatch[2]; // Return process ID
    }

    // Look for process-specific patterns
