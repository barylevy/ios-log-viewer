...
const ColumnSettings = ({
  ...
  useEffect(() => {
    if (isOpen) setTempColumns(visibleColumns);
  ...
  const handleReset = () => {
    ...
    if (typeof onResetDefaults === 'function') onResetDefaults();
  };
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4 max-h-[90vh] flex flex-col">
        ...
        <div className="p-4 space-y-2 flex-1 overflow-y-auto">
          <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
            Toggle column visibility. To reorder or resize columns, drag the column headers in the log view.
          </p>

          {AVAILABLE_COLUMNS.map((column) => (
            <div
              key={column.id}
              className="flex items-center justify-between p-3 rounded-md bg-gray-50 dark:bg-gray-900 hover:bg-gray-100 dark:hover:bg-gray-800"
            >
              <label
